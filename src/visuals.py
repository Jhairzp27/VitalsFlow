...
class Visualizer:
    ...
    def _apply_base_layout(self, fig):
    ...
    def plot_q1_trends(self, df: pd.DataFrame, window: int = 3):
    ...
    def plot_q2_top_hospitals(self, df: pd.DataFrame):
        df_q2 = df.groupby('Hospital')['Billing Amount'].sum().reset_index().sort_values('Billing Amount', ascending=False).head(10)
        fig = px.bar(df_q2, x='Billing Amount', y='Hospital', orientation='h', 
                     text=df_q2['Billing Amount'].apply(lambda x: f"${x/1000000:.1f}M"),
                     color='Billing Amount', color_continuous_scale='Blues')
        fig.update_traces(textposition='inside')
        fig.update_layout(yaxis={'categoryorder':'total ascending'}, xaxis_title="Ingresos Totales ($)", yaxis_title="", coloraxis_showscale=False)
        return self._apply_base_layout(fig)

    def plot_q3_avg_stay(self, df: pd.DataFrame, group_col: str):
        df_q3 = df.groupby(group_col)['Days Hospitalized'].mean().reset_index()
        fig = px.bar(df_q3, x=group_col, y='Days Hospitalized', text=df_q3['Days Hospitalized'].round(1),
                     color=group_col, color_discrete_sequence=px.colors.qualitative.Pastel)
    ...
    def plot_q4_test_results(self, df: pd.DataFrame):
        ...
        fig = go.Figure(data=[go.Pie(labels=df_q4['Resultado'], values=df_q4['Cantidad'], pull=pull_values, hole=0.4, marker_colors=['#FF5A5F', '#00A699', '#FFAA00'])])
        return self._apply_base_layout(fig)

    def plot_q5_cost_distribution(self, df: pd.DataFrame):
        median_order = df.groupby('Medical Condition')['Billing Amount'].median().sort_values(ascending=False).index
        fig = px.box(df, x='Medical Condition', y='Billing Amount', color='Medical Condition', 
